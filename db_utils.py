...
def append_vectors_to_table(db: lancedb.DBConnection, table_name: str, vector_video, vector_audio, filenames: list,
    ...
    if not (len(vector_video) == len(filenames) == len(vector_audio)):
        raise ValueError(
            f"Length mismatch: vector ({len(vector_video)}),"
